...
int next_bat() 
{
  ...
  if (AC_on()) {               // charging... 
    ...
    if (CM4_active == 1) {     //   with CM4 active, batVoltage[] readings are valid... so pick the lowest voltage (valid) battery
      ...
      for (n=0; n<4; n++) {
        if ((batVoltage[n] < lowest_voltage) && (batVoltage[n] > 3100)) {  // batt > 3100 to be selected
          nextBat = n;
 ...
 * Vref = Vcc (about 4.35V); so LSB = 4.25mV; 4v *2.2K / (2.2K + 10K) = 0.72v;  = 169
 */
bool AC_on()       // Verified
{  
  Charger_Vcc = (analogRead(VCC_SENSE));  // 169 calculates to charger == 4.0 V
  if (Charger_Vcc > 169) {
     return (true);
  }
  return (false);
...
void DataReceive(int numBytes)
{
  ...
  // Handle the battery voltage write by putting the value to the appropriate batVoltage array member
    if ((rcvReg >= 0x21) && (rcvReg <= 0x24)){
      batVoltage[rcvReg - 0x21] = rcvVal * 16;
    }
